...
if [[ "$HOSTNAME" == "pbody" ]]; then
	...
	scp -v ~/*.tar box1@192.168.10.2:/run/media/box1/backups/ | tee -a $LOG
		if [ "$?" = "0" ]; then
			echo
...
curl http://textbelt.com/text -d "number=$ME" -d "message=$HOSTNAME scp failed. Backup failed." |tee -a $LOG
			exit 1
		fi
else
	echo
	echo "This is box1, copying to external." | tee -a $LOG
	mv -v ~/*.tar /run/media/box1/backups/ | tee -a $LOG
	find /run/media/box1/backups/*backup*.tar -mtime +14 -exec rm {} \; | tee -a $LOG
fi
...
curl http://textbelt.com/text -d "number=$ME" -d "message=$HOSTNAME backup is complete. $SIZE MB" | tee -a $LOG
